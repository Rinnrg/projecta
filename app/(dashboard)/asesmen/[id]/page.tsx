...
async function getAsesmenCourseId(asesmenId: string) {
  const asesmen = await prisma.asesmen.findUnique({
    where: { id: asesmenId },
    select: { courseId: true },
  })
  return asesmen?.courseId
}
...
export default async function AsesmenDetailPage({ params, searchParams }: PageProps) {
  ...
  // Get courseId from asesmen if not in searchParams
  const targetCourseId = courseId || await getAsesmenCourseId(id)
  
  if (!targetCourseId) {
    redirect('/asesmen')
  }
  
  // Redirect to nested route in courses
  redirect(`/courses/${targetCourseId}/asesmen/${id}`)
}
...
export default async function AsesmenDetailPage({ params, searchParams }: PageProps) {
  ...
  if (!courseId) {
    ...
    if (asesmen) {
      redirect(`/courses/${asesmen.courseId}/asesmen/${id}`)
    }
  }
  
  // Redirect to nested route
  redirect(`/courses/${courseId}/asesmen/${id}`)
      nilai: {
        include: {
          siswa: {
            select: {
