...
export default function AddProyekPage({ params }: { params: Promise<{ sintaks: string }> }) {
  ...
  const [tglSelesai, setTglSelesai] = useState<Date>()
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const data = {
        ...
        lampiran: lampiran && !lampiran.startsWith('data:') ? lampiran : null,
        fileData,
        fileName,
        fileType,
        fileSize
    ...
    <div className="space-y-6">
      ...
      <div className="max-w-3xl mx-auto">
        <form onSubmit={handleSubmit}>
          <Card>
            <CardHeader>
              <CardTitle>Form Tahapan Proyek Baru</CardTitle>
              <CardDescription>
                Menambahkan tahapan: <span className="font-semibold">{judulProyek}</span>
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                <Label>Tahapan Proyek</Label>
                <div className="px-3 py-2 border rounded-md bg-muted text-muted-foreground">
                  {judulProyek}
                </div>
                <p className="text-xs text-muted-foreground">
                  Otomatis berdasarkan tahapan sintaks
                </p>
              </div>
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      ...
                      >
                        ...
                        {tglMulai ? format(tglMulai, "PPP", { locale: indonesia }) : <span>Pilih tanggal mulai tahapan ini</span>}
                      </Button>
                    </PopoverTrigger>
                ...
                <div className="space-y-2 flex flex-col">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      ...
                      >
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {tglSelesai ? format(tglSelesai, "PPP", { locale: indonesia }) : <span>Pilih tanggal selesai tahapan ini</span>}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0" align="start">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <Button
                  type="submit"
                  className="flex-1"
                  disabled={loading || !tglMulai || !tglSelesai}
                >
                  {loading ? (
                    <>Menyimpan...</>
                  ) : (
